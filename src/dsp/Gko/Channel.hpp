...
struct Channel {
  ...
  inline Layer* newRecording() {
    assert(_record_interface.active());
    assert(_recording_layer == nullptr);

    unsigned int n_beats = 1;
    if (_record_interface.mode == RecordInterface::Mode::DUB && 0 < _timeline.layers.size()) {
    ...
    if (_record_interface.time_frame != TimeFrame::TIMELINE) {
