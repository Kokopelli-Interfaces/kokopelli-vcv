...
//   LiveNode* recording_member = new LiveNode(_timeline_position, n_beats, _selected_members_idx, _signal_type, samples_per_beat);
...
void Engine::startLoving() {
  // TODO if (love_self)
  LiveNode* new_member = new LiveNode();
...
void Engine::stopLoving() {
  // TODO make focused member inactive
    LiveNode* loved_member = CircleAccessor::getMember(_music_circle, state.focused_member_id))
